import sys as s, linecache as l, time, os
dynamicAttempt = 0
dynamicLine = 1
totalLine = 0
...
class General:
    ...
    def getDisplayContent(self, action):
        ...
        if action == 'dynamicLine:+1': dynamicLine+=1
        elif action == 'dynamicLine:Current': dynamicLine=dynamicLine
        elif action == 'dynamicLine:CheckingPassword': dynamicLine=getCurrentLineToCheckPassword
        elif action == 'dynamicLine:Reset': dynamicLine=1
    ...
    def accountSucceedWriteToDB(self):
        ...
        if displayName == 'undefined' or displayUser == 'undefined' or displayPassword == 'undefined':
            with open('auth/lib/userdb.txt', 'w') as f:
                f.write(f'{userInName}|{userInUsername}|{userInPassword}|'), f.close()
        else: 
...
class Validation:
    ...
    def contentPassCheckLoginData(self, action):
        ...
        if action == 'from:CheckUserContent':
            ...
            while userNameLoginExistBool == False:
                if dynamicLine != totalLine:
                    if displayUser == userInUsername: userNameLoginExistBool = True
                    elif onCheckFirst == True: 
                        General().getDisplayContent('dynamicLine:Reset')
                        onCheckFirst = False
                    else: General().getDisplayContent('dynamicLine:+1')
                else: 
            ...
            while passwordMatchUserBool == False:
                if displayPassword == userInPassword: passwordMatchUserBool = True
                else: break
        ...
        if action == 'from:CheckUserContent':
            if userNameLoginExistBool == False:
                ...
                print(f'Username ⨉ {3-dynamicAttempt} {"try" if 3-dynamicAttempt==1 else "tries"} remain{"s" if dynamicAttempt == 2 else ""}.')
                Login().userContentUsername()
        elif action == 'check:CheckPasswordContent':
            if passwordMatchUserBool == False:
                General().getUserAttemptDynamic('Login:Fail', '')
                General().formatUI(2)
                print(f'Username ✓ Password ⨉ {3-dynamicAttempt} {"try" if 3-dynamicAttempt==1 else "tries"} remain{"s" if dynamicAttempt == 2 else ""}.')
                Login().userContentPassword()
    ...
    def validateRegisterInUser(self, action):
        if action == 'valid:name':
            if len(userInName) <= 3 or userInName is any(x.isdigit() for x in userInName) or ' ' not in userInName:
                ...
                General().formatUI(5)
                print(f'Name ⨉ {3-dynamicAttempt} {"try" if 3-dynamicAttempt==1 else "tries"} remain{"s" if dynamicAttempt == 2 else ""}.')
                Register().userContentName()
        elif action == 'valid:userName':
            if len(userInUsername) < 4 or ' ' in userInUsername: 
                General().getUserAttemptDynamic('CreateAccount:Fail', 'from:Registration')
                General().formatUI(6)
                print(f'Name ✓ Username ⨉ {3-dynamicAttempt} {"try" if 3-dynamicAttempt==1 else "tries"} remain{"s" if dynamicAttempt == 2 else ""}.')
                Register().userContentUsername()
            else: Validation().checkRegisterUserInData('from:CheckUserContent')
            ...
            if (len(userInPassword) < 8 and len(userInPassword) > 60) or not any(x.isupper() for x in userInPassword) or not any(x.isdigit() for x in userInPassword): 
                General().getUserAttemptDynamic('CreateAccount:Fail', 'from:Registration')
                General().formatUI(6)
                print(f'Name ✓ Username ✓ Password ⨉ {3-dynamicAttempt} {"try" if 3-dynamicAttempt==1 else "tries"} remain{"s" if dynamicAttempt == 2 else ""}.')
                Register().userContentPassword()
    ...
    def checkRegisterUserInData(self, action):
        if action == 'from:CheckUserContent':
            ...
            while duplicateUsernameBool == False:
                if dynamicLine != totalLine:
                    if displayUser == userInUsername: duplicateUsernameBool = True
                    else: General().getDisplayContent('dynamicLine:+1')
                else: 
                    General().getDisplayContent('dynamicLine:Current')
                    if displayUser == userInUsername: duplicateUsernameBool = True
    ...
    def clearUserDataActing(self):
        ...
        print(f'Database check ✓ User Input ✓ 5 Accounts {"✓" if totalLine >= 5 else "⨉"}')

        if (displayUser != userInUsername and totalLine < 5) or totalLine < 5:
            decisionContinue = False
            if (displayUser == userInUsername and totalLine < 5): errorReason = 'Not enough users'
            elif (displayUser != userInUsername and totalLine >= 5): errorReason = 'Not First User'
            else: errorReason = 'You did not meet any of the requirements.'
        ...
        if decisionContinue == True:
            userInContinueRemoveData = input('Are you sure you want to remove everyone, apart from yourself, from the database? (Yes/No)\n').title()
            if userInContinueRemoveData == 'Yes':
                with open('auth/lib/userdb.txt', 'w') as f:
                    f.write(f'{displayName}|{displayUser}|{displayPassword}')
                    f.close()
                print('Successfully Removed.')
                askUserRegister = input('Do you want to register a new account? (Yes/No)').title()
...
try:
    action  = input("How's it gonna be? Login or Register?\nI'm going to ").lower()
    if action in 'register': 
        os.system('cls' if os.name == 'nt' else "printf '\033c'")
        Register().userContentRegisterBase('r')
