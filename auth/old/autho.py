...
class Register:
        ...
        while auth == False:
            ...
            if password == c_password:
                for i in range(2):
                    ...
                    i
                print(
                    f"name ✓ username ✓ Password ✓ Password Confirmation ✓"), time.sleep(.1)
    ...
    def wFile_newUser(self, name, userName, password):
        ...
        with open('Auth/Inc/login.txt', 'a') as contents:
            if l.getline('Auth/Inc/Login.txt', 1) == '':
                contents.write(
                    f'{name}|{userName}|{password}|'), contents.close()
            else:
                contents.write(
    ...
    def reg_req(self, redir):
        ...
        if redir == 'f-getN':
            print(l.getline(loc, 1))
        elif redir == 'f-getUN':
            print(l.getline(loc, 2))
        elif redir == 'f-getP':
            print(l.getline(loc, 3))
        print(f"{'-'*50}")
...
class Login:
    ...
    def login(self):
        ...
        Login().userName()
        Login().password()
        s.stdout.write("\033[F"), print(
            "Username ✓ Password ✓ | Account Authorised ✓")
        with open('Auth/Inc/currentUser.txt', 'w') as f:
            f.write(f'{sys_name}|{userName}|'), f.close()
        time.sleep(1)
        s.stdout.write("\033[F"), s.stdout.write(
            "\033[K"), print(f'Status: Logged in')
        for i in range(3):
    ...
    def auth_data(self, a):
        ...
        if xerr == True:
            ...
            if a == 'check:password':
                ...
                General().clearLnR(rng=2)
                print(
                    f'username ✓ password ⨉ {3-u_attempt} {"try" if 3 - u_attempt == 1 else "tries"} remain')
                Login().password()
            elif a == 'check:user':
                General().clearLnR(rng=3)
                print(
                    f'username ⨉ {3-u_attempt} {"try" if 3 - u_attempt == 1 else "tries"} remain')
                Login().userName()
...
class Validation:
    ...
    def chk_data(self):
        ...
        if sys_name == 'undefined' or sys_userName == 'undefined' or sys_password == 'undefined':
            ...
            time.sleep(3), os.system(
                'cls' if os.name == 'nt' else "printf '\033c'")
            Register().register(a='reg')
    ...
    def rmvDataVal(self, userName):
        ...
        if dataBlock[1] != userName and line < 5:
            conRmvData = False, print(
        ...
        if conRmvData == True:
            while True:
                cfrmDelData = input(
                    'Are you sure you want to remove all user data? (Yes/No)\n').title()
                if cfrmDelData == 'Yes':
                    with open('Auth/Inc/Login.txt', 'w') as f:
                        f.write(f'{sys_name}|{userName}|{password}'), f.close()
    ...
    def uRemove_def(self):
        ...
        General().totline()
        if (os.path.isfile('Auth/Inc/login.txt') and os.access('Auth/Inc/login.txt', os.R_OK)) and sys_name == 'undefined':
    ...
    def chk_exist_user(self,):
        if os.path.isfile('Auth/Inc/login.txt') and os.access('Auth/Inc/login.txt', os.R_OK):
            ...
            if xerr == True:
                General().attempts(pathDecision=False, goto='')
                print(
                    f'name ✓ username ⨉ {3-u_attempt} {"try" if 3 - u_attempt == 1 else "tries"} remain')
                Register().userName(re='')
...
try:
    ...
    if a in 'register':
        os.system('cls' if os.name ==
                  'nt' else "printf '\033c'"), Register().register(a)
    elif a == 'x-rmv-aldata':
        os.system('cls' if os.name == 'nt' else "printf '\033c'"), Login(
        ).clrUserData(a='x-rmv-aldata')
    else:
        os.system('cls' if os.name == 'nt' else "printf '\033c'"), Validation(
        ).chk_data(), Login().login()
