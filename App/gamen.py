#imports
import linecache as l, sys as s, random, time, os
...
class General:
    ...
    def UIdata(self):
        ...
        userDecisionTypeMusic = input(f"Please select from the following options to begin playing: {typeofData}\n").title()
        if userDecisionTypeMusic == typeofData:
            General().getTotalLines(f'{typeofData}.txt', 'app/lib/standard-quizes')
            questionRangeIndex = list(range(1, totalLine))
            input(f'Okay, running {typeofData} music now.\nPress ENTER to continue.'), Game().userContentStandardQuiz()
        else: print(f'"{userDecisionTypeMusic}" is not a music genre avaliable for this quiz.\nRestart the program and try again.')
    ...
    def getDisplayContentUser(self):
        ...
        if os.path.isfile(f'{folderDirectoryQuery}/{fileDirectoryQuery}') and os.access(f'{folderDirectoryQuery}/{fileDirectoryQuery}', os.R_OK):
            databaseContent = l.getline(f'{folderDirectoryQuery}/{fileDirectoryQuery}', 1)
    ...
    def getDisplayContentSystem(self, action, typeofAction):
        ...
        if action == 'GetStandardQuizData':
            ...
            readLineFromFile = 1
        elif action == 'GetUserGeneratedQuiz': pass #NOT YET IMP.
        
        if typeofAction == 'standardQuiz': folderDirectoryQuery = 'app/lib/standard-quizes'
        elif typeofAction == 'GetUserGeneratedQuiz': folderDirectoryQuery = 'app/lib/ugen-quizes'
        
        if os.path.isfile(f'{folderDirectoryQuery}/{fileDirectoryQuery}') and os.access(f'{folderDirectoryQuery}/{fileDirectoryQuery}', os.R_OK):
            databaseContent = l.getline(f'{folderDirectoryQuery}/{fileDirectoryQuery}', readLineFromFile)
            individualContent = databaseContent.split('|')
            ...
            if action == 'GetStandardQuizData':
                ...
                typeofData = individualContent[0] # To change get back all avail options
        else: ProcessFeatures().newFileGenerate(fileDirectoryQuery, folderDirectoryQuery)
    ...
    def getDisplayContentScoring(self, action):
        ...
        if os.path.isfile(f'{folderDirectoryQuery}/{fileDirectoryQuery}') and os.access(f'{folderDirectoryQuery}/{fileDirectoryQuery}', os.R_OK):
            databaseContent = l.getline(f'{fileDirectoryQuery}', readLineFromFile)
            individualContent = databaseContent.split('|')
...
class Game:
    def userContentStandardQuiz(self):
        ...
        while questionIterateContinue == True:
            ...
            General().getTotalLines(f'{userDecisionTypeMusic}.txt', 'app/lib/standard-quizes')
            if displaySong == None: 
                input(f'{displayFirstName}, you have completed this set of questions.\nCurrent Score is {dynamicScore}.\nPress ENTER to exit.')
                questionIterateContinue = True

            questionNotCorrect = True
            while questionNotCorrect == True:
                questionPresentUser = input(f'Question {dynamicQuestionInt}\n{"-"*50}\nFirst Letter of Song: {displayFirstStringItem} | Artist: {displayArtist}\n{"-"*20}\nSong Name: ').title()
                if questionPresentUser == displaySong:
                    matchXBool = 'questionMatch:True'
                    ScoreCheck().userDynamicScoreUpdate('questionMatch:True'), ScoreCheck().userHighScoreValidate('Check:HighScoreData')
                ...
                if matchXBool != 'questionMatch:True':
                    ...
                    if dynamicAttempt == 3:
                        ...
                        break
                    userInputIncorrectMoveOn = input(f'You didn\'t get the answer correct. Do you want to try one more time? (Yes/No)\nYou have {3-dynamicAttempt} tr{"ies" if 3-dynamicAttempt>=2 else "y"} left before moving on.\n').title()
                    if userInputIncorrectMoveOn == 'Yes': pass
                    elif userInputIncorrectMoveOn == 'No': 
                        questionNotCorrect = False
                        questionWrong += 1
                        General().getUserAttemptDynamic('UserInputCorrect:ResetAttempts')
                    else:
                        print('Nothing was entered. We presume you want to try again...')

                if matchXBool == 'questionMatch:True':
                    questionPresentUserReleaseYear = input(f'Do you know what year "{displaySong}" by "{displayArtist}" was released?\n{"-"*20}\nRelease Year: ')
                    if questionPresentUserReleaseYear == str(displaySongReleaseYear):  ScoreCheck().userDynamicScoreUpdate('releaseYearMatch:True'), ScoreCheck().userHighScoreValidate('Check:HighScoreData')

            if questionRangeIndex == []:
                os.system('cls' if os.name=='nt' else 'clear')
                while True:
                    endOfSetDecision = input(f'{displayFirstName}, that is the end of the set! Want to play again? (yes/no)\n').lower()
                    if endOfSetDecision == 'yes': Game().userContentStandardQuiz()
                    elif endOfSetDecision == 'no':
                        print(f'Thanks for playing!\nRemember your username "{displayUser}" for next time!')
                        questionIterateContinue = False
                        break
                    else: General().getUserAttemptDynamic('userInputNotValid:+1')
...
class ScoreCheck:
    ...
    def userDynamicScoreUpdate(self, action):
        global dynamicScore
        General().getDisplayContentUser()
        if action == 'questionMatch:True': dynamicScore+=1
        elif action == 'releaseYearMatch:True': dynamicScore+=2
        elif action == 'getCurrentScore:True': return dynamicScore
    ...
    def userHighScoreValidate(self, action):
        ...
        if action == 'Check:HighScoreData':
            if displayUser == 'undefined' and displayHighScore == 'undefined':
                ...
                with open(f'app/lib/', 'w') as f: f.write(f'{currentDayValue}|{dynamicScore}|{displayUser}|{displayName}\n'), f.close()
            elif dynamicScore > displayHighScore and displayUsername==displayUser:
                print("You just beat your own score! Well done!")
                with open(f'App/Score/{userDecisionTypeMusic}.txt', 'w') as f: f.write(f'{currentDayValue}|{dynamicScore}|{displayUser}|{displayName}'), f.close()
...
class ProcessFeatures:
    ...
    def newFileGenerate(self, fileDirectoryQuery, folderDirectoryQuery):
        with open(f'{folderDirectoryQuery}/{fileDirectoryQuery}.txt', 'w') as f: f.write(f'{"undefined|"*4}'), f.close()
